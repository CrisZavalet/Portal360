...
export class FichajeEmpleado {
  ...
  cambiarEstado(fichaje: HistorialFichajeEmpleado, nuevoEstado: string): void {
    const estado = nuevoEstado === 'NULL' ? null : nuevoEstado;

    console.log('ID fichaje:', fichaje.idClocking);
    console.log('Nuevo estado:', estado);

    this.authService.updateAprobadoStatus(fichaje.idClocking, estado as string).subscribe({
      next: (respuesta) => {
  ...
  exportarPDF(datos: HistorialFichajeEmpleado[]) {
    ...
    pdf.setFontSize(10);
    pdf.text(
      'Empleado: ' + this.empleadoselectado?.name + ' ' + this.empleadoselectado?.lastName,
      14,
      22,
    );
    autoTable(pdf, {
      startY: 30,
      head: [['Fecha', 'Entrada', 'Salida', 'Tiempo', 'Estado', 'Aprobado']],
  ...
  aprobarFichajesMes(): void {
    const fichajesMes = this.fichajesFiltrados;

    fichajesMes.forEach((fichaje) => {
      this.authService.updateAprobadoStatus(fichaje.idClocking, 'APROBADO').subscribe({
        next: () => {
          fichaje.aprobado = 'APROBADO';
        },
        error: (error) => {
          console.error(`Error aprobando fichaje ${fichaje.idClocking}`, error);
        },
      });
    });
  }
